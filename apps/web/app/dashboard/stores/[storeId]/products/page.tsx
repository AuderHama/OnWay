...
export default function Home() {
  ...
  const { data, isPending, isError, error, refetch } = useQuery({
    ...
    enabled: !!storeId,
    queryFn: async () => {
      const res = await fetch(`/api/v1/stores/${storeId}/products`);
      if (!res.ok) {
  ...
  const createProduct = useMutation({
    mutationFn: async (body: {
      ...
      const res = await fetch(`/api/v1/stores/${storeId}/product`, {
        method: "POST",
          ...
          <form
            onSubmit={(e) => {
              ...
              if (submitMode == "insert") {
                createProduct.mutate({ name, price, isAvailable });
              } else {
                updateProduct.mutate({ id, name, price, isAvailable });
              }
            }}
          >
            <DialogHeader>
